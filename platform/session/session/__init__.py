...
It composes an existing auth.AuthStore and tenancy.TenancyStore into one
call:

    svc = SessionService(auth_store, tenancy_store)
...
`../auth` and `../tenancy` are put on sys.path here (the same trick both of
those modules use for their own dependencies).
"""
