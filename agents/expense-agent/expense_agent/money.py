"""Deterministic parsing of a single money value out of model output.

CLAUDE.md rule #4: the model transcribes what it sees on the receipt (a string
or a JSON number); turning that into an exact `Decimal` for the policy checks is
plain code, with its own tests. Nothing here rounds or computes — it only
normalizes a single scalar.

This is a near-copy of `agents/ap-agent/ap_agent/money.py`. Kept local so this
session touches only one agent folder (CLAUDE.md rule #5). FOLLOW-UP: consolidate
both copies into a small shared `platform/` helper.
"""
