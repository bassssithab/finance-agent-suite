...
Same idea as agents/close-agent/evals/fakes.py: regulatory-change-agent's one
LLM call (impact-assessment narrative) forces a `record_impact_assessment` tool
call, so the fake returns a `tool_use` block and records every request for
assertions.
"""
...
def assessment_client(payload: dict) -> FakeAnthropicClient:
    return FakeAnthropicClient(FakeMessage(
        content=[FakeToolUseBlock(name="record_impact_assessment", input=payload)],
...
def no_tool_call_client() -> FakeAnthropicClient:
    return FakeAnthropicClient(FakeMessage(
        content=[FakeTextBlock(text="Here is my view in prose.")], stop_reason="end_turn",
    ))
