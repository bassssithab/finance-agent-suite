...
Same idea as agents/close-agent/evals/fakes.py: controls-sox-agent's one LLM
call (deficiency-narrative drafting) forces a `record_deficiency_narratives`
tool call, so the fake returns a `tool_use` block and records every request for
assertions.
"""
...
def narratives_client(payload: dict) -> FakeAnthropicClient:
    return FakeAnthropicClient(FakeMessage(
        content=[FakeToolUseBlock(name="record_deficiency_narratives", input=payload)],
