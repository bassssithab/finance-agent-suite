...
Same idea as agents/close-agent/evals/fakes.py: ar-collections-agent's one LLM
call (dunning-email drafting) forces a `record_dunning_drafts` tool call, so the
fake returns a `tool_use` block and records every request for assertions.
"""
...
def dunning_client(payload: dict) -> FakeAnthropicClient:
    return FakeAnthropicClient(FakeMessage(
        content=[FakeToolUseBlock(name="record_dunning_drafts", input=payload)],
