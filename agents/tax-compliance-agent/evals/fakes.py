...
Same idea as agents/close-agent/evals/fakes.py: tax-compliance-agent's one LLM
call (filing-support narrative) forces a `record_filing_support_narrative` tool
...
def narrative_client(payload: dict) -> FakeAnthropicClient:
    return FakeAnthropicClient(FakeMessage(
        content=[FakeToolUseBlock(name="record_filing_support_narrative", input=payload)],
